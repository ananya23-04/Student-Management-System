...
class reportClass:
    def __init__(self, root):
        ...
        # Title
        title = Label(self.root, text="View Student Results", font=("goudy old style", 20, "bold"), background="orange", foreground="#262626").place(x=10, y=15, width=1180, height=50)
        ...
        btn_search=Button(self.root,text="Search",font=("goudy old style",15,"bold"),background="#03a9f4",foreground="white",cursor="hand2",command=self.search).place(x=680,y=100,width=100,height=35)
        btn_clear=Button(self.root,text="Clear",font=("goudy old style",15,"bold"),background="gray",foreground="white",cursor="hand2",command=self.clear).place(x=800,y=100,width=100,height=35)
        ...
        #===Button delete===
        btn_delete=Button(self.root,text="Delete",font=("goudy old style",15,"bold"),background="red",foreground="white",cursor="hand2",command=self.delete).place(x=500,y=350,width=150,height=35)
    ...
    def delete(self):
        ...
        try:
            if self.var_id=="":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Inavlid Student result",parent=self.root)
                else:
                    op=messagebox.askyesno("Confirm","Do you really want to delete?",parent=self.root)
                    if op==True:
                        cur.execute("Delete from result where rid=?",(self.var_id,))
                        con.commit()
                        messagebox.showinfo("Delete","Result Deleted Successfullly",parent=self.root)
                        self.clear()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to {str(ex)}")
