...
class Clock:
         ...
         try:
             ...
             if row==None:
                messagebox.showerror("Error","Please enter the valid email address to reset your password",parent=self.root)
                 ...
                 #=================Forget Password
                 question=Label(self.root2,text="Securtiy Question",font=("times new roman",15,"bold"),bg="white",fg="gray").place(x=50,y=100)
    ...
    def login(self):
       if self.txt_email.get()=="" or self.txt_pass_.get()=="":
          messagebox.showerror("Error","All Fields are reguired",parent=self.root)
       else:
          try:
             con=sqlite3.connect(database="rms.db")
             cur=con.cursor()
